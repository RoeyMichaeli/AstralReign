...
struct PlayerConfig { short SR, CR, OR, OG, OB, FR, FG, FB, OT, V; float S; };
struct EnemyConfig  { short SR, CR, OR, OG, OB,				OT, VMIN, VMAX, L, SI; float SMIN, SMAX; };
struct BulletConfig { short SR, CR, OR, OG, OB, FR, FG, FB, OT, V; float S; };

struct EnemySpawnArea { short minX, minY, maxX, maxY; };
...
class Game {
	...
	int init(const std::string& path)
	{
		...
		m_window.setFramerateLimit(60); // 33ms per frame
		...
		m_enemySpawnArea.minX = m_enemyConfig.SR * 1.5;
		m_enemySpawnArea.minY = m_enemyConfig.SR * 1.5;
		m_enemySpawnArea.maxX = m_screenSize.x - m_enemySpawnArea.minX;
		m_enemySpawnArea.maxY = m_screenSize.y - m_enemySpawnArea.minY;
	...
	void sMovement()
	{
		...
		for (auto entity : enemyView)
		{
			...
			float radius = m_enemyConfig.SR * 1.5;
		...
		//playerTransform.position.x += playerTransform.velocity.x * (playerInput.right - playerInput.left);
			...
			for (auto [entity2, transform2, col2] : view.each()) {
				...
				if (entity1 < entity2) {
					...
					if (distSquared < sumRadii * sumRadii) {
						...
						if (m_registry.all_of<TPlayer>(entity1) && (m_registry.all_of<TEnemy>(entity2) || m_registry.all_of<TSmallEnemy>(entity2))
							|| m_registry.all_of<TPlayer>(entity2) && (m_registry.all_of<TEnemy>(entity1) || m_registry.all_of<TSmallEnemy>(entity1))) {
						...
						else if (m_registry.all_of<TBullet>(entity1) && (m_registry.all_of<TEnemy>(entity2) || m_registry.all_of<TSmallEnemy>(entity2))
							|| m_registry.all_of<TBullet>(entity2) && (m_registry.all_of<TEnemy>(entity1) || m_registry.all_of<TSmallEnemy>(entity1))) {
							m_registry.emplace<TPendingDestruction>(entity1);
							m_registry.emplace<TPendingDestruction>(entity2);
							m_score += 100;
						}
	...
	void spawnPlayer()
	{
		...
		m_registry.emplace<CColission>(player, m_playerConfig.CR + m_playerConfig.OT);
	...
	void spawnEnemy()
	{
		...
		m_registry.emplace<CTransform>(enemy,
			// Position
			Vector2f((rand() % (1 + m_enemySpawnArea.maxX - m_enemySpawnArea.minX)) + m_enemySpawnArea.minX,
				(rand() % (1 + m_enemySpawnArea.maxY - m_enemySpawnArea.minY)) + m_enemySpawnArea.minY),
			// Velocity
			Vector2f((static_cast<float> (rand()) / static_cast<float>(RAND_MAX)) * (m_enemyConfig.SMAX - m_enemyConfig.SMIN),
				(static_cast<float> (rand()) / static_cast<float>(RAND_MAX)) * (m_enemyConfig.SMAX - m_enemyConfig.SMIN)),
			// Angle
			static_cast<float> (rand()) / 360 // Angle
		);
		...
		m_registry.emplace<CShape>(enemy,
			...
			sf::Color(rand() % 255, rand() % 255, rand() % 255), // Fill Color
			sf::Color(m_enemyConfig.OR, m_enemyConfig.OG, m_enemyConfig.OB), // Outline Color
			m_enemyConfig.OT	// Outline Thickness
		);

		m_registry.emplace<CColission>(enemy, m_enemyConfig.CR + m_enemyConfig.OT);
